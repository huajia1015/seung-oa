...
<div class="main_content">
		...
		<form action="select-student-sign-in.html" method="post" id="pageForm_ID" class="niceform">
		<input type="hidden" id="current_ID" name="current" value="${page.current }"/>
		<input type="hidden" name="status" value="${signIn.status }"/>
		<input type="hidden" name="signDateStr" value="<fmt:formatDate value="${signIn.signDate}" pattern="yyyy-MM-dd"/>"/>
		<fieldset>
				<dl style="margin-left:10%;">
                 ...
                 <dl style="margin-left:10%;">
					...
					<dd>
                     ...
                     <!-- <dt style="width:10px;"><label for="courseCode">到:</label></dt> -->
					<dd>
                 ...
                 <dl>
					<dd style="margin-left:30%;"><a href="javascript:void();" id="selectBtn" class="bt_blue"><span class="bt_blue_lft"></span><strong>查 询</strong><span class="bt_blue_r"></span></a></dd>
                 </dl>
        </fieldset>
		</form>
		</div>
    ...
    <c:forEach items="${page.list}" var="row">
			<tr>
				...
				<td>${row.lesson }小时</td>
				<td><c:if test="${row.status=='1' }">已签到</c:if><c:if test="${row.status=='2' }">缺勤</c:if></td>
				<td>${row.remarks }</td>
			</tr>
			</c:forEach>
    </tbody>
		...
		<pg:page url="select-student-sign-in.html"></pg:page>
		</div>
		</c:if>
		<!-- 无记录 -->
		<c:if test="${empty page.list}">
